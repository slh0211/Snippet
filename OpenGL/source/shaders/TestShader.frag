...
Purpose: frag shader for phong shading
Language: C++/glsl
...
void main()
{
  ...
  if(objectID == mainId)
  {
    ...
    {
        case backID:
            frag_color = texture(backTex, uv);
            //frag_color = vec4(1.0, 0.0, 1.0, 1.0); 
            break;
        
        case rightID:
            frag_color = texture(rightTex, uv);
            break;

        case frontID:
            frag_color = texture(frontTex, uv);
            break;

        case leftID:
            frag_color = texture(leftTex, uv);
            break;

        case topID:
            frag_color = texture(topTex, uv);
            break;

        case bottomID:
            frag_color = texture(bottomTex, uv);
            break;
    }
