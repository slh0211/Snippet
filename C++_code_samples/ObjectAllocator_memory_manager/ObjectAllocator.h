...
/*!
  ...
  \brief  
    The ObjectAllocator class implements a fixed size memory allocator

    Operations include:

    - Allocate memory for client
    - Free the item and put back to the free list
    - Dump whole block when there is a leak
    - Check if blocks are corrupted
*/
/******************************************************************************/
...
class ObjectAllocator
{
  ...
  private:
      // static const for validating pad
    static const int UNDER_RUN = -1;
    static const int OVER_RUN  = 1;
      // static const for extra credit
    static const bool EXTRA    = true;
    ...
    void AllocatePage();
      // Check the given address is on the free list
    bool IsOnFreeList(void *Object) const;
      // Check the given address is on the given page
    bool IsOnCurrentPage(void *Object, void *Page) const;
      // Check the given address is at right boundary
    bool IsAtRightBoundary(void *Object, void *Page) const;
      // Check if the entire page is empty
    bool IsAnEmptyPage(void *Page) const;
      // Check if the pad been overwritten
    int  ValidPad(void *Object) const;
      // Remove given object from free list
    void RemoveFromFreeList(void *Object);
      // Remove every object on the page from free list
    void RemoveAllFreeObject(void *Page);
      // Calculate and return left most block's size
    unsigned GetLeftBlockSize() const;
      // Calculate and return inter block's size
    unsigned GetInterBlockSize() const;

    GenericObject* pPageList_;        // Head of pages
    GenericObject* pFreeList_;        // Head of free objects
};
